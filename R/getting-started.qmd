...
[^getting-started-6]: <https://github.com/intro-to-behds/behdsbook-part-1>
...
In RStudio, you can navigate to the *Tools* tab and select install packages for CRAN packages such as **tidyverse** -- this dialog does not support GitHub-only packages like **behdslabs**, which must be installed with `remotes::install_github` as shown above. We can then load a package into our R sessions using the `library` function:
...
We can install more than one CRAN package at once by feeding a character vector to `install.packages`:
