...
Let's look at one more example using the US data-breach data frame:
...
Here is a very simple example that tells us the state with the lowest data-breach incident rate if it is lower than 0.5 per 100,000. The if-else statement protects us from the case in which no state satisfies the condition.

```{r}
ind <- which.min(incident_rate)

if (incident_rate[ind] < 0.5) {
  print(app_data_breaches$state[ind])
} else{
  print("No state has an incident rate that low")
}
```
...
```{r}
no_nas <- ifelse(is.na(daily_mood_ratings), 0, daily_mood_ratings)
sum(is.na(no_nas))
```
