...
We say that a data table is in  _tidy_ format if each row represents one observation and columns represent the different variables available for each of these observations. The `app_data_breaches` dataset is an example of a tidy data frame.
...
Each row represent a state with each of the five columns providing a different variable related to these states: name, abbreviation, region, population, and number of reported incidents.

To see how the same information can be provided in different formats, consider the following example:

```{r, echo=FALSE}
tidy_data <- global_tech_adoption |> 
  filter(country %in% c("South Korea", "Germany") & !is.na(avg_price_per_app_usd)) |>
  select(country, year, avg_price_per_app_usd)
head(tidy_data, 6)
```

This tidy dataset provides average app prices for two countries across the years. This is a tidy dataset because each row represents one observation with the three variables being country, year, and average app price. However, this dataset originally came in another format and was reshaped for the __behdslabs__ package. Originally, the data was in the following format:


```{r, echo=FALSE, message=FALSE}
path <- system.file("extdata", package = "behdslabs")
filename <- file.path(path, "app-price-two-countries-example.csv")
...
We want all the necessary information for our analysis to be included in the data frame. The first task is to add the incident rates to our data-breach data frame.  The function `mutate`  takes the data frame as a first argument and the name and values of the variable as a second argument using the convention `name = values`. So, to add incident rates, we use:
 
```{r, message=FALSE}
app_data_breaches <- mutate(app_data_breaches, rate = incidents/population*100000)
```

Notice that here we used `incidents` and `population` inside the function, which are objects that are **not** defined in our workspace. But why don't we get an error?

This is one of __dplyr__'s main features. Functions in this package, such as `mutate`, know to look for variables in the data frame provided in the first argument. In the call to mutate above, `incidents` will have the values in `app_data_breaches$incidents`. This approach makes the code much more readable. 
...
Now suppose that we want to filter the data frame to only show the entries for which the incident rate is lower than 1.42. To do this we use the `filter` function, which takes the data frame as the first argument and then a conditional statement as the second. Like `mutate`, we can use the unquoted variable names from `app_data_breaches` inside the function and it will know we mean the columns and not objects in the workspace.

```{r}
filter(app_data_breaches, rate <= 1.42)
```
...
```{r}
new_dataframe <- select(app_data_breaches, state, region, rate)
filter(new_dataframe, rate <= 1.42)
```

In the call to `select`, the first argument `app_data_breaches` is an object, but `state`, `region`, and `rate` are variable names. 
...
```{r}
new_dataframe <- select(app_data_breaches, where(is.numeric))
names(new_dataframe)
```
...
```{r}
new_dataframe <- select(app_data_breaches, starts_with("r"))
names(new_dataframe)
```
...
```{r}
#| eval: false
mutate(app_data_breaches, population = log10(population))
```

Often, we need to apply the same transformation to several variables. The function `across` facilitates the operation. For example if want to log transform both population and incident counts we can use:

```{r}
#| eval: false
mutate(app_data_breaches, across(c(population, incidents), log10))
```
...
```{r}
#| eval: false
mutate(app_data_breaches, across(where(is.numeric), log10))
```
...
```{r}
#| eval: false
mutate(app_data_breaches, across(where(is.character), tolower))
```
...
```{r}
app_data_breaches |> select(state, region, rate) |> filter(rate <= 1.42)
```
...
app_data_breaches |> select(state, region, rate) |> filter(rate <= 1.42)
```
`app_data_breaches` is the first argument of the `select` function, and the new data frame (formerly `new_dataframe`) is the first argument of the `filter` function.
...
The `summarize` function in __dplyr__ provides a way to compute summary statistics with intuitive and readable code. We start with a simple example based on reaction times. The `reaction_times` dataset includes reaction times (in milliseconds) and sex recorded in a simple response task. 
...
s <- reaction_times |> 
  filter(sex == "Female") |>
  summarize(average = mean(rt_ms), standard_deviation = sd(rt_ms))
s
```

This takes our original data frame as input, filters it to keep only females, and then produces a new summarized table with just the average and the standard deviation of reaction times. We get to choose the names of the columns of the resulting table. For example, above we decided to use `average` and `standard_deviation`, but we could have used other names just the same.
...
As with most other __dplyr__ functions, `summarize` is aware of the variable names and we can use them directly. So when inside the call to the `summarize` function we write `mean(rt_ms)`, the function is accessing the column with the name "rt_ms" and then computing the average of the resulting numeric vector. We can compute any other summary that operates on vectors and returns a single value. 

For another example of how we can use the `summarize` function, let's compute the average incident rate for the United States. Remember our data table includes reported incidents and population size for each state and we have already used __dplyr__ to add a incident rate column:

```{r}
app_data_breaches <- app_data_breaches |> mutate(rate = incidents/population*100000)
```

Remember that the US incident rate is **not** the average of the state incident rates:
...
This is because in the computation above, the small states are given the same weight as the large ones. The US incident rate is the total number of incidents in the US divided by the total US population. So the correct computation is:

```{r}
us_incident_rate <- app_data_breaches |>
  summarize(rate = sum(incidents)/sum(population)*100000)
us_incident_rate
```
...
Suppose we want three summaries from the same variable such as the median, minimum, and maximum reaction times. We can use `summarize` like this:
 
```{r}
reaction_times |> summarize(median = median(rt_ms), min = min(rt_ms), max = max(rt_ms))
```
...
```{r}
reaction_times |> reframe(quantiles = quantile(rt_ms, c(0.5, 0, 1)))
```
...
For another example, let's compute the median, minimum, and maximum incident rate in the four regions of the country using the `median_min_max` defined above:
...
The `us_incident_rate` object defined in @sec-summarize represents just one number. Yet we are storing it in a data frame:

```{r}
class(us_incident_rate)
```
...
```{r}
app_data_breaches |> arrange(population) |> head()
```

With `arrange` we get to decide which column to sort by. To see the states sorted by incident rates, for example, we would use `arrange(rate)` instead.

Note that the default behavior is to order in ascending order. In __dplyr__, the function `desc` transforms a vector so that it is in descending order. To sort the data frame by incident rates in descending order, we can type:

```{r, eval=FALSE}
app_data_breaches |> arrange(desc(rate)) 
```

### Nested sorting 

If we are ordering by a column with ties, we can use a second column to break the tie. Similarly, a third column can be used to break ties between first and second and so on. Here we order by `region`, then within region we order by incident rate:
...
In the code above, we have used the function `head` to avoid having the page fill up with the entire dataset. For instance, using `arrange(desc(rate))` followed by `head` would show the 6 states with the largest incident rates, in order. Instead, to view a specific number of observations with the highest incident rates, we can use the `slice_max` function. This function takes a data frame as it's first argument, the number of rows to show in the second, and the variable to filter by in the third. Here is an example of how to see the top 5 rows:

```{r}
app_data_breaches |> slice_max(rate, n = 5)
```
...
```{r}
app_data_breaches |> group_by(region) |> class()
```
...
The print method for tibbles is more readable than that of a data frame. To see this, compare the outputs of typing `app_data_breaches` and the output of `app_data_breaches` if we convert it to a tibble. We can do this using `as_tibble(app_data_breaches)`. If using RStudio, output for a tibble adjusts to your window size. To see this, change the width of your R console and notice how more/less columns are shown.
...
```{r}
class(app_data_breaches[,4])
```

is not a data frame. With tibbles this does not happen:

```{r}
class(as_tibble(app_data_breaches)[,4])
```
...
```{r}
class(as_tibble(app_data_breaches)$population)
```
...
```{r}
as_tibble(app_data_breaches)$Population
```
...
A common use for this function is to define categorical variables based on existing variables. For example, suppose we want to compare the incident rates in four groups of states: _New England_, _West Coast_, _South_,  and _other_. For each state, we need to ask if it is in New England, if it is not we ask if it is in the West Coast, if not we ask if it is in the South, and if not we assign _other_. Here is how we use `case_when` to do this:
...
app_data_breaches |> 
  mutate(group = case_when(
    ...
    TRUE ~ "Other")) |>
  group_by(group) |>
  summarize(rate = sum(incidents)/sum(population)*10^5) 
```
...
5\. Load the __dplyr__ package and the data-breach dataset.
...
```{r, eval=FALSE}
app_data_breaches <- mutate(app_data_breaches, population_in_millions = population/10^6)
```

We can write `population` rather than `app_data_breaches$population`. The function `mutate` knows we are grabbing columns from `app_data_breaches`.

Use the function `mutate` to add a column named `rate` with the per 100,000 incident rate as in the example code above. Make sure you redefine `app_data_breaches` as done in the example code above (`app_data_breaches <- [your code]`) so we can keep using this variable.

6\. If `rank(x)` gives you the ranks of `x` from lowest to highest, `rank(-x)` gives you the ranks from highest to lowest. Use the function `mutate` to add a column `rank` containing the rank of incident rate from highest to lowest. Make sure you redefine `app_data_breaches` so we can keep using this variable.
...
```{r, eval=FALSE}
select(app_data_breaches, state, population)
```

Use `select` to show the state names and abbreviations in `app_data_breaches`. Do not redefine `app_data_breaches`, just show the results.
...
```{r, eval=FALSE}
filter(app_data_breaches, state == "New York")
```

You can use other logical vectors to filter rows.

Use `filter` to show the top 5 states with the highest incident rates. From here on, do not change the data-breach dataset, just show the result. Remember that you can filter based on the `rank` column.

 
9\. We can remove rows using the `!=` operator. For example, to remove Florida, we would do this:

```{r, eval=FALSE}
no_florida <- filter(app_data_breaches, state != "Florida")
```
...
```{r, eval=FALSE}
filter(app_data_breaches, state %in% c("New York", "Texas"))
```
...
11\. Suppose you want to live in the Northeast or West **and** want the incident rate to be less than 2. We want to see the data for the states satisfying these options. Note that you can use logical operators like `&` with `filter`. Here is an example in which we filter to keep only small states in the Northeast region.

```{r, eval=FALSE}
filter(app_data_breaches, population < 5000000 & region == "Northeast")
```

Make sure `app_data_breaches` has been defined with `rate` and `rank` and still has all states. Create a table called `my_states` that contains rows for states satisfying both the conditions: it is in the Northeast or West and the incident rate is less than 2.  Use `select` to show only the state name, the rate, and the rank.


12\. The pipe `|>` can be used to perform operations sequentially without having to define intermediate objects. Start by redefining `app_data_breaches` to include rate and rank.
...
```{r, eval=FALSE}
my_states <- filter(app_data_breaches, region %in% c("Northeast", "West") &
                      rate < 2)
...
```{r, eval=FALSE}
mutate(app_data_breaches, rate =  incidents/population*100000, 
       rank = rank(-rate)) |>
...
sd(daily_mood_ratings, na.rm = TRUE)
```
...
19\. Load the `app_data_breaches` dataset. Which of the following is true?

a. `app_data_breaches` is in tidy format and is stored in a tibble.
b. `app_data_breaches` is in tidy format and is stored in a data frame.
c. `app_data_breaches` is not in tidy format and is stored in a tibble.
d. `app_data_breaches` is not in tidy format and is stored in a data frame.
    
20\. Use `as_tibble` to convert the `app_data_breaches` data table into a tibble and save it in an object called `breaches_tibble`.
    
21\. Use the `group_by` function to convert `app_data_breaches` into a tibble that is grouped by region.

22\. Write tidyverse code that is equivalent to this code:
    
```{r, eval=FALSE}
exp(mean(log(app_data_breaches$population)))
```

Write it using the pipe so that each function is called without arguments. Use `pull` to access the population variable. Hint: The code should start with `app_data_breaches |>`.
