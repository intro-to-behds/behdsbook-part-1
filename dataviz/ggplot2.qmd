...
We will construct a graph that summarizes the app data-breaches dataset that looks like this:
...
app_data_breaches |> ggplot(aes(x = population/10^6, y = incidents, label = abb)) +
  ...
  ylab("Total number of incidents (log scale)") +
  ggtitle("Reported Data-Breach Incidents by US State") +
  scale_color_discrete(name = "Region") +
  theme_economist()
```

We can clearly see how much states vary across population size and the total number of data-breach incidents. Not surprisingly, we also see a clear relationship between incident totals and population size. A state falling on the dashed grey line has the same incident rate as the US average. The four geographic regions are denoted with color, which depicts how most southern states have incident rates above the average.
...
-   **Data**: The `app_data_breaches` data frame is being summarized. We refer to this as the **data** component.
...
-   **Aesthetic mapping**: The plot uses several visual cues to represent the information provided by the dataset. The two most important cues in this plot are the point positions on the x-axis and y-axis, which represent population size and the total number of incidents, respectively. Each point represents a different observation, and we *map* data about these observations to visual cues like x- and y-scale. Color is another visual cue that we map to region. We refer to this as the **aesthetic mapping** component. How we define the mapping depends on what **geometry** we are using.
...
We start by loading the relevant dataset which is in the **behdslabs package**:
...
```{r}
#| eval: false
ggplot(data = app_data_breaches)
```

or equivalently

```{r ggplot-example-1}
app_data_breaches |> ggplot()
```
...
```{r}
p <- ggplot(data = app_data_breaches)
```
...
To summarize, `p` is a `ggplot` object with the `app_data_breaches` data frame as its data component.
...
*Aesthetic mappings* describe how properties of the data connect with features of the graph, such as distance along an axis, size, or color. The `aes` function connects data with what we see on the graph by defining aesthetic mappings and will be one of the functions you use most often when plotting. This example produces a scatterplot of total incidents versus population in millions:

```{r ggplot-example-2}
app_data_breaches |> ggplot() + geom_point(aes(population/10^6, incidents))
```
...
The scale and labels are defined by default when adding this layer. Like **dplyr** functions, `aes` also uses the variable names from the data component: we can use `population` and `incidents` without having to call them as `app_data_breaches$population` and `app_data_breaches$incidents`. The behavior of recognizing the variables from the data component is specific to `aes`. With **ggplot2** functions other than `aes`, if you try to access the values of `population` or `incidents`, you receive an error.
...
```{r  ggplot-example-3}
app_data_breaches |> ggplot() +
  geom_point(aes(population/10^6, incidents)) +
  geom_text(aes(population/10^6, incidents, label = abb))
```

As an example of the unique behavior of `aes` related to variable names, note that if the added layer was `geom_text(aes(population/10^6, incidents), label = abb)`, we would result in an error since `abb` is now outside the call to `aes` and it is not object in out workspace, it is a variable name in the data component of the `ggplot` object.

## Global aesthetic mappings

In the previous lines of code, we define the mapping `aes(population/10^6, incidents)` twice, once in each layer. We can avoid this by using a *global* aesthetic mapping. We can do this when we define the blank slate `ggplot` object. Remember that the `mapping` argument of `ggplot` function permits us to define aesthetic mappings. If we define a mapping in `ggplot`, all the geometries that are added as layers will default to this mapping. So we can simply write the following code to produce the previous plot:

```{r}
#| eval: false
app_data_breaches |> ggplot(aes(population/10^6, incidents)) +
  geom_point() +
...
Clearly, the second call to `geom_text` does not use `population` and `incidents`.
...
```{r}
p0 <- app_data_breaches |> ggplot(aes(population/10^6, incidents))
```
...
```{r ggplot-example-9}
p4 <- p3 + labs(title = "Reported Data-Breach Incidents by US State",
                x = "Populations in millions (log scale)",
                y = "Total number of incidents (log scale)",
                color = "Region")
p4
```

Our desired final plot includes a line that represents the average incident rate for the entire country. Once we determine the per million rate to be $r$, the desired line is defined by the formula: $y = r x$, with $y$ and $x$ our axes: total incidents and population in millions, respectively. In the log-scale this line turns into: $\log(y) = \log(r) + \log(x)$, a line with slope 1 and intercept $\log(r)$. We can compute `r` using:

```{r}
r <- app_data_breaches |>
  summarize(rate = sum(incidents)/sum(population)*10^6) |>
  pull(rate)
```
...
The style of a **ggplot2** graph can be changed using the `theme` functions. Several themes are included as part of the **ggplot2** package. In fact, for most of the plots in this book, we use a function in the **behdslabs** package that automatically sets a default theme:
...
app_data_breaches |>
  ggplot(aes(population/10^6, incidents)) +
  geom_abline(intercept = log10(r), lty = 2, color = "darkgrey") +
  geom_point(aes(col = region), size = 3) +
  geom_text_repel(aes(label = abb)) +
  scale_x_log10() +
  scale_y_log10() +
  labs(title = "Reported Data-Breach Incidents by US State",
                x = "Populations in millions (log scale)",
                y = "Total number of incidents (log scale)",
                color = "Region") +
  theme_economist()
...
```{r barplot-geom}
#| eval: false
app_data_breaches |> ggplot(aes(region)) + geom_bar()
```
...
Note that we use the optional arguments `bandwidth = 10` to change the bin size to 10 milliseconds. The default is to create 30 bins. We also use the optional arguments `fill = "blue"` and `col = "black"` to fill the bars with colors and use a different color to outline the bars.
...
The geometry for boxplot is `geom_boxplot`. As discussed, boxplots are useful for comparing distributions. For example, below are the previously shown reaction times for women, but compared to men. For this geometry, we need arguments `x` as the categories, and `y` as the values.

```{r female-male-boxplots-geom, eval=FALSE}
reaction_times |> ggplot(aes(sex, rt_ms)) +
  geom_boxplot()
```
...
Start by loading the **dplyr** and **ggplot2** library as well as the `app_data_breaches` and `reaction_times` data.
...
```{r, eval = FALSE}
p <- ggplot(app_data_breaches)
```

and we can also use the pipe:

```{r, eval = FALSE}
p <- app_data_breaches |> ggplot()
```
...
3\. Using the pipe `|>`, create an object `p` but this time associated with the `reaction_times` dataset instead of the `app_data_breaches` dataset.

4\. What is the class of the object `p` you have just created?

5\. Now we are going to add a layer and the corresponding aesthetic mappings. For the app_data_breaches data we plotted total incidents versus population sizes. Explore the `app_data_breaches` data frame to remind yourself what are the names for these two variables and select the correct answer. **Hint**: Look at `?app_data_breaches`.

a.  `state` and `abb`.
b.  `total_incidents` and `population_size`.
c.  `incidents` and `population`.
d.  `app_data_breaches` and `size`.
...
```{r, eval=FALSE}
app_data_breaches |> ggplot(aes(x = , y = )) +
  geom_point()
```
...
```{r, eval=FALSE}
app_data_breaches |> ggplot(aes(population, incidents)) +
  geom_point()
```

Remake the plot but now with incidents on the x-axis and population on the y-axis.

8\. If instead of points we want to add text, we can use the `geom_text()` or `geom_label()` geometries. The following code

```{r, eval=FALSE}
app_data_breaches |> ggplot(aes(population, incidents)) + geom_label()
```
...
a.  Adding a column called `blue` to `app_data_breaches`.
...
a.  Adding a column called `color` to `app_data_breaches` with the color we want to use.
...
```{r, eval=FALSE}
p <- app_data_breaches |>
  ggplot(aes(population, incidents, label = abb, color = region)) +
  geom_label()
```
...
16\. Now edit the code above to add the title "Data breach incidents" to the plot. Hint: use the `ggtitle` function.

17\. Now we are going to use the `geom_histogram` function to make a histogram of the reaction times in the `reaction_times` data frame. When reading the documentation for this function we see that it requires just one mapping, the values to be used for the histogram. Make a histogram of all the plots.

What is the variable containing the reaction times?

a.  `sex`
b.  `reaction_times`
c.  `rt_ms`
d.  `reaction_times$rt_ms`

18\. Now create a ggplot object using the pipe to assign the reaction-time data to a ggplot object. Assign `rt_ms` to the x values through the `aes` function.
...
Use the `binwidth` argument to change the histogram made in the previous exercise to use bins of size 10 milliseconds.
