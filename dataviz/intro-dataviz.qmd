...
cognitive_battery_scores |>
  ...
  annotate("rect", xmin = 65, xmax = 99, ymin = 0, ymax = 28000, alpha = .5) +
  geom_bar(stat = "identity", color = "black", fill = "#C4843C") +
  annotate("text", x = 66, y = 22400, label = "\"TYPICAL RANGE\"\nBENCHMARK\nSCORE IS 65", hjust = 0, size = 3) +
  annotate("text", x = 0, y = 9600, label = "Scores on\nthe five cognitive-battery tasks", hjust = 0, size = 3) +
...
if (knitr::is_html_output()) {
  ...
  p <- filter(global_tech_adoption, year %in% years & !is.na(group) &
    ...
    geom_text(aes(x = 10.5, y = 9.5, label = year), cex = 12, color = "grey") +
    facet_grid(. ~ year)
