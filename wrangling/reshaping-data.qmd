...
We will use the app-price wide format dataset described in @sec-tidy-data as an example in this section.
...
As with most tidyverse functions, the `pivot_longer` function's first argument is the data frame that will be converted. Here we want to reshape the `wide_data` dataset so that each row represents an app-price observation, which implies we need three columns to store the year, country, and the observed value. In its current form, data from different years are in different columns with the year values stored in the column names. Through the `names_to` and `values_to` argument we will tell `pivot_longer` the column names we want to assign to the columns containing the current column names and observations, respectively. The default names are `name` and `value`, which are often usable choices. In this case a better choice for these two arguments would be `year` and `app_price`. Note that nowhere in the data file does it tell us this is app-price data. Instead, we deciphered this from the file name. Through `cols`, the second argument, we specify the columns containing observed values; these are the columns that will be *pivoted*. The default is to pivot all columns so, in most cases, we have to specify the columns. In our example we want columns `1960`, `1961` up to `2015`.

The code to pivot the app-price data therefore looks like this:

```{r}
new_tidy_data <- wide_data |>
  pivot_longer(`1960`:`2015`, names_to = "year", values_to = "app_price")
```

We can see that the data have been converted to tidy format with columns `year` and `app_price`
...
```{r}
new_tidy_data <- wide_data |>
  pivot_longer(-country, names_to = "year", values_to = "app_price")
```
...
tidy_data <- global_tech_adoption |>
  ...
  select(country, year, app_price = avg_price_per_app_usd)
```
...
```{r app-price-year-check, eval=FALSE}
new_tidy_data |> 
  ggplot(aes(year, app_price, color = country)) + 
  geom_point()
```
...
new_wide_data <- new_tidy_data |> 
  pivot_wider(names_from = year, values_from = app_price)
...
The data wrangling shown above was simple compared to what is usually required. In our example spreadsheet files, we include an illustration that is slightly more complicated. It contains two variables: screen time and app price. However, the way it is stored is not tidy and, as we will explain, not optimal.

```{r, message=FALSE}
path <- system.file("extdata", package = "behdslabs")

filename <- "screentime-and-app-price-two-countries-example.csv"
...
The result is not exactly what we refer to as tidy since each observation is associated with two, not one, rows. We want to have the values from the two variables, app price and screen time, in two separate columns. The first challenge to achieve this is to separate the `name` column into the year and the variable type. Notice that the entries in this column separate the year from the variable name with an underscore:
...
The data is now in tidy format with one row for each observation with three variables: year, app price, and screen time.
...
wide_data <- read_csv(filename)
new_tidy_data <- wide_data |>
  pivot_longer(-1, names_to = "year", values_to = "app_price")
```
...
```{r message = FALSE}
library(janitor)
names(dat) <- c("Country", "Year", "App Price",  "Screen Time")
...
4\. Now load the `app_task_completion` data set, which contains task-completion information for the mobile and desktop versions of an app across six tasks, and keep only the completion-rate column:

```{r, eval=FALSE}
dat <- app_task_completion |> select(-n_attempts)
```

If we think of an observation as a task, and that each observation has two variables (mobile completion rate and desktop completion rate) then this is not tidy. Use the `pivot_wider` function to wrangle into tidy shape: one row for each task.

5\. Now we will try a more advanced wrangling challenge. We want to wrangle the task-completion data so that for each task we have 4 observations: `completion_rate_mobile`, `completion_rate_desktop`, `n_attempts_mobile` and `n_attempts_desktop`. The *trick* we perform here is actually quite common: first use `pivot_longer` to generate an intermediate data frame and then `pivot_wider` to obtain the tidy data we want. We will go step by step in this and the next two exercises.

Use the `pivot_longer` function to create a `tmp` data frame with a column containing the type of observation: `completion_rate` or `n_attempts`. Call the new columns `name` and `value`.

6\. Now you have an object `tmp` with columns `task`, `device`, `name` and `value`. Note that if you combine the `name` and `device`, we get the column names we want: `completion_rate_mobile`, `completion_rate_desktop`, `n_attempts_mobile` and `n_attempts_desktop`. Use the function `unite` to create a new column called `column_name`.

7\. Now use the `pivot_wider` function to generate the tidy data with four variables for each task.

8\. Now use the pipe to write a line of code that turns `app_task_completion` to the table produced in the previous exercise.
