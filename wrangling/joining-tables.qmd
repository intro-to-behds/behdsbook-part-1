...
Suppose we want to explore the relationship between population size for US states and how much weight each state carries in a company's product-launch rollout plan. We have the population size in this table:
...
and the rollout weights in this one:

```{r}
head(product_launch_results)
```

Just concatenating these two tables together will not work since the order of the states is not the same (and here the two tables don't even use the same column name for the state — `state` in one, `market` in the other):

```{r}
identical(product_launch_results$market, app_data_breaches$state)
```
...
The _join_ functions in the __dplyr__ package make sure that the tables are combined so that matching rows are together. If you know SQL, you will see that the approach and syntax is very similar. The general idea is that one needs to identify one or more columns that will serve to match the two tables. When the matching column has a different name in each table, `by` takes a named vector (`by = c("state" = "market")`) so we can still tell the two functions to line up on this shared key. Notice what happens if we join the two tables above using `left_join` (we will remove the `adopt_other` column and rename `market_weight` so that the tables fit on the page):

```{r}
tab <- left_join(app_data_breaches, product_launch_results, by = c("state" = "market")) |>
  select(-adopt_other) |> rename(mw = market_weight)
head(tab)
```

The data has been successfully joined and we can now, for example, make a plot to explore the relationship:

```{r mw-vs-population, message=FALSE, warning=FALSE, echo=FALSE}
library(ggrepel)
tab |> ggplot(aes(population/10^6, mw, label = abb)) +
  geom_point() +
...
We see the relationship is close to linear with about 2 rollout-weight units for every million persons, but with very small states getting higher ratios.


In practice, it is not always the case that each row in one table has a matching row in the other. For this reason, we have several versions of join. To illustrate this challenge, we will take subsets of the tables above. We create the tables `tab1` and `tab2` so that they have some states in common but not all (renaming `market` to `state` here so both subsets share a common column name, as is often good practice when preparing tables you know you'll join):

```{r}
tab_1 <- slice(app_data_breaches, 1:6) |> select(state, population)
tab_2 <- product_launch_results |>
  filter(market %in% c("Alabama", "Alaska", "Arizona",
                    "California", "Connecticut", "Delaware")) |>
  select(state = market, market_weight) |> rename(mw = market_weight)
```
...
In **data.table** the `merge` functions works similarly (using `by.x`/`by.y` since the two tables' key columns have different names):

```{r}
#| message: false
#| warning: false
library(data.table)
tab <- merge(app_data_breaches, product_launch_results, by.x = "state", by.y = "market", all.x = TRUE)
```
