...
the `app_data_breaches`, `reaction_times`, and `research_funding_rates` examples into the
...
Most of the examples will come from the first case study which deals
with self-reported daily screen time, and most of the chapter is
...
## Case study 1: self-reported daily screen time

The **behdslabs** package includes the raw data from which a "daily
screen time in minutes" variable can be obtained. You can load it like
this:
...
These screen-time values were obtained using a web form in which
respondents were asked to enter their average daily screen time. They
could enter anything, but the instructions asked for *screen time in
minutes*, a number. We compiled
`r prettyNum(nrow(reported_screen_time), big.mark=",")` submissions, but
unfortunately the column vector with the reported screen time had
several non-numeric entries and as a result became a character vector:

```{r}
class(reported_screen_time$screen_time)
```

If we try to parse it into numbers, we get a warning:

```{r}
x <- as.numeric(reported_screen_time$screen_time)
```

Although most values appear to be screen time in minutes as requested
...
reported_screen_time |>
  mutate(new_screen_time = as.numeric(screen_time)) |>
  filter(is.na(new_screen_time)) |>
  head(n = 10)
```

We immediately see what is happening. Some respondents did not report
their screen time in minutes as requested. We could discard these data
and continue. However, many of the entries follow patterns that, in
principle, we can easily convert to minutes. For example, in the output
above, we see various cases that use the format `x'y"` or `x'y''` with `x` and `y`
representing hours and minutes, respectively -- a shorthand some
respondents apparently borrowed from feet-and-inches height notation.
Each one of these cases can be read and converted to minutes by a
human, for example `2'15"` is `2*60 + 15 = 135`. So we could fix all the
problematic entries *by hand*. However, humans are prone to making
mistakes, so an automated approach is preferable. Also, because we plan
on continuing to collect data, it will be convenient to write code that
automatically corrects entries entered in error.
...
consistent with being in minutes and to view only the problematic
...
outside a range of plausible daily screen-time values. We permit a
fairly wide range, from 15 minutes to 10 hours a day. We also use
`suppressWarnings` to avoid the warning message we know `as.numeric`
will gives us.
...
representing hours and minutes, respectively. Here are ten examples:
...
2\. A pattern of the form `x.y` or `x,y` with `x` hours and `y` minutes,
written as if the two were joined by a decimal point instead of a
colon. Here are ten examples:

```{r, echo=FALSE}
pattern <- "^[1-9]\\s*[\\.|,]\\s*([0-9]|[0-5][0-9])$"
str_subset(problems, pattern) |> head(n = 10) |> cat()
```

3\. Entries that were reported in seconds rather than minutes. Here
are ten examples:

```{r, echo=FALSE}
ind <- which(between(suppressWarnings(as.numeric(problems))/60, 15, 600) )
ind <- ind[!is.na(ind)]
...
1.  Convert entries fitting the first two patterns into one standardized
    one.
2.  Leverage the standardization to extract the hours and minutes and
    convert to minutes.
3.  Define a procedure for identifying entries that are in seconds
...
Now, what if we want our string to be 5 hours written like this `5'`? In
...
So we've learned how to write 5 hours and 10 minutes separately, but what
if we want to write them together to represent *5 hours and 10 minutes*
...
Above, we noted that an entry included seconds far too large to be
minutes. Let's instead look for entries that spelled out the unit --
we can show all the entries that used `sec` like this:

```{r}
str_subset(reported_screen_time$screen_time, "sec")
```

### Special characters

Now let's consider a slightly more complicated example. Which of the
following strings contain the pattern `sec` or `hours`?

```{r}
yes <- c("300 sec", "2 hours")
no <- c("300", "2h")
s <- c(yes, no)
```
...
```{r}
str_detect(s, "sec") | str_detect(s, "hours")
```
...
`|` which means *or*. So if we want to know if either `sec` or `hours`
appears in the strings, we can use the regex `sec|hours`:

```{r}
str_detect(s, "sec|hours")
```

and obtain the correct answer.

Another special character that will be useful for identifying hours and
minutes values is `\d` which means any digit: 0, 1, 2, 3, 4, 5, 6, 7, 8,
...
useful in our case study since the hours part of a daily screen-time
report is never more than 1 digit so a restriction will help us. One
way to do this with regex is by using
...
For the minutes part, we can have one or two digits. This can be
...
our hours and minutes pattern, we can add the symbols for hours `'` and
minutes `"` after the digits.

With what we have learned, we can now construct an example for the
pattern `x'y"` with `x` hours and `y` minutes.
...
-   `^` = start of the string
-   `[1-9]` = one digit, from 1 through 9
-   `'` = hours symbol
...
```{r}
yes <- c("5'7\"", "6'2\"",  "5'70\"")
no <- c("6,2\"", "6.2\"","I am 5'11\"", "0'2\"", "64")
str_detect(yes, pattern)
str_detect(no, pattern)
```

For now, we are permitting the minutes to be 60 or larger. We will add a
...
We will actually use all three in our reported screen time example, but we
...
We want to change screen times written like `5.6` to `5'6`.

To avoid changing patterns such as `70.2`, we will require that the
first digit be between 1 and 9 `[1-9]` and that the second be none or
more digits `\\d*`. Let's start by defining a simple pattern that
matches this:

```{r}
pattern_without_groups <- "^[1-9],\\d*$"
```
...
Notice that the second and third columns contain hours and minutes,
...
not appear to be in minutes. We can see that not too many of our
...
An initial problem we see immediately is that some respondents wrote out
the words "hours" and "minutes" (or abbreviations like "hr" and "min").
We can see the entries that did this with the `str_subset` function:

```{r}
str_subset(problems, "min")
```
...
hours and minutes with a uniform symbol. We will use `'` for hours, whereas
for minutes we will simply not use a symbol since some entries were of
the form `x'y`. Now, if we no longer use the minutes symbol, we have to
...
```{r}
problems |>
  str_replace("hours|hour|hrs|hr", "'") |> # replace hours, hour, hr with '
  str_replace("minutes|minute|mins|min|''|\"", "") |> # remove all minutes symbols
  str_detect(pattern) |>
  sum()
```
...
the hours symbol `'` to permit space between the hours symbol and the
numbers. Now we match a few more entries:

```{r}
pattern <- "^[1-9]\\s*'\\s*\\d{1,2}$"
problems |>
  str_replace("hours|hour|hrs|hr", "'") |> # replace hours, hour, hr with '
  str_replace("minutes|minute|mins|min|''|\"", "") |> # remove all minutes symbols
  str_detect(pattern) |>
...
sure that it does not have unintended effects. In our reported screen
time examples, this will be a problem because some entries are of the
form `x y` with space separating the hours from the minutes. If we
remove all spaces, we will incorrectly turn `x y` into `xy` which
implies that a `6 1` would become `61` minutes instead of `361`.
...
a very specific pattern that assures us hours and minutes are being
...
We can use this to convert cases in our reported screen time.
...
```{r}
pattern_with_groups <- "^([1-9])\\s*[,\\.\\s+]\\s*(\\d*)$"
```

Let's break this one down:

-   `^` = start of the string
-   `[1-9]` = one digit representing hours, from 1 through 9
-   `\\s*` = none or more white space
-   `[,\\.\\s+]` = hours and minutes are separated by either `,`, `.` or at least one space
-   `\\s*` = none or more white space
-   `\\d*` = none or more digits representing minutes
-   `$` = end of the string
...
Again, we will deal with the minutes-larger-than-sixty challenge later.
...
```{r}
patterns <- c(hours = "\\d", "\\s*'\\s*", minutes = "\\d{1,2}", ".*")
...
`Two hours fifteen minutes`. Although not efficient, we could add many extra
...
We are now ready to put it all together and wrangle our reported screen time data to try to recover as many values as possible. The code is complex, but we will break it down into parts.

Let's see how many problematic entries we can recover with the approaches we covered in @sec-regex. We will first define a function that detects entries that are not reported as minutes or seconds:

```{r}
not_minutes_or_seconds <- function(x, smallest = 15, tallest = 600){
  minutes <- suppressWarnings(as.numeric(x))
  is.na(minutes) |
    !((minutes >= smallest & minutes <= tallest) |
        (minutes/60 >= smallest & minutes/60 <= tallest))
}
```

We can see how many entries are not minutes or seconds:

```{r}
problems <- reported_screen_time |>
  filter(not_minutes_or_seconds(screen_time)) |>
  pull(screen_time)
length(problems)
```

Let's see how many hours`'`minutes format we can recover applying the finding from @sec-regex. Specifically we replace hours/hour/hr/hrs with `'`, we remove the word minutes and its abbreviations, then rewrite similar patterns into the desired hours`'`minutes pattern. Once this is done we see what proportion don't fit the desired pattern

```{r}
converted <- problems |>
  str_replace("hours|hour|hrs|hr", "'") |>
  str_remove_all("minutes|minute|mins|min|''|\"") |>
  str_replace("^([1-9])\\s*[,\\.\\s+]\\s*(\\d*)$", "\\1'\\2")
index <- str_detect(converted, "^[1-9]\\s*'\\s*\\d{1,2}$")
mean(!index)
```
...
We noticed that some respondents added `sec` (or spelled it out entirely, `seconds`) and some entries have space at the end so we incorporate this into our cleanup stage. We also notice that at least one entry wrote out numbers such as `Two hours fifteen minutes`. We can use the `words()` function in the **english** package to change these to actual numbers. In preparation for our final product, we define a function that cleans up previously noticed problems and these new ones:

```{r, warning=FALSE, message=FALSE}
library(english)
cleanup <- function(s){
  s <- str_remove_all(s, "minutes|minute|mins|min|''|\"|seconds|secs|sec|and") |>
    str_trim() |>
    str_to_lower()
  for (i in 59:0) {
...
Note that we loop from 59 down to 0 rather than the other way around: a number word like "fourteen" contains the word "four" as a substring, so if we replaced `four` first we would corrupt `fourteen` before ever getting a chance to recognize it. Counting down avoids this, since every two-word or teen number (10 and up) is handled before the single digits it might contain.

Many also notice that respondents reporting exactly a whole number of hours did not enter any minutes, for example `2'`, and our pattern requires that minutes be included, and that some entries are decimal hours below 1, and some of these use European decimals, for example `0.5` and `0,63`. So we
create a function that add these corrections to those already identified as needed to reformat the entry as hours`'`minutes:

```{r}
convert_format <- function(s){
  s |> str_replace("hours|hour|hrs|hr", "'") |>
    str_replace("^([1-9])\\s*[,\\.\\s+]\\s*(\\d*)$", "\\1'\\2") |>
    str_replace("^([1-9])'?$", "\\1'0") |>
    str_replace("^(0)\\s*,\\s*(\\d*)$", "\\1\\.\\2")
}
```

Now we are ready to wrangle our reported screen time dataset. The strategy is as follows:

1\. We start by defining a variable to keep a copy of the original entry, we then clean up and covert the `screen_time` entry using the functions described above.

2\. We then use the `separate_wider_regex_` function to extract hours and minutes when the entry matches our hours`'`minutes format.

3\. We use a lookaround to make sure that entries that have numbers with no `'` following them don't match and return an `NA`.

4.\ Once this is done, we convert the hours and minutes into minutes.

5.\ Finally, we decide if the entries are in minutes, seconds, or decimal hours and convert appropriately.


```{r, warning=FALSE, message=FALSE}
patterns <- c(hours = "[1-9](?=\\s*'\\s*)",
              "\\s*'\\s*",
              minutes = "\\d+\\.?\\d*")
smallest <- 15
tallest <- 600
new_screen_time <- reported_screen_time |>
  mutate(original = screen_time,
         screen_time = convert_format(cleanup(screen_time))) |>
  separate_wider_regex(screen_time, patterns = patterns,
                       too_few = "align_start",
                       cols_remove = FALSE) |>
  mutate(across(c(screen_time, hours, minutes), as.numeric)) |>
  mutate(guess = 60 * hours + minutes) |>
  mutate(screen_time = case_when(
    is.na(screen_time) ~ as.numeric(NA),
    between(screen_time, smallest, tallest) ~ screen_time,  #minutes
    between(screen_time/60, smallest, tallest) ~ screen_time/60, #seconds
    between(screen_time*60, smallest, tallest) ~ screen_time*60, #decimal hours
    TRUE ~ as.numeric(NA))) |>
  mutate(screen_time = ifelse(is.na(screen_time) &
                           minutes <= 60 & between(guess, smallest, tallest),
                         guess, screen_time)) |>
  select(-hours, -minutes, -guess)
```

We see that we fix all but `r sum(is.na(new_screen_time$screen_time))` entries. You can see that these are mostly un-fixable:

```{r}
new_screen_time |> filter(is.na(screen_time)) |> pull(original)
```
...
#| eval: false
new_screen_time |>
  filter(not_minutes_or_seconds(original)) |>
  select(original, screen_time) |>
  arrange(screen_time) |>
  View()
```

A final observation is that if we look at the respondents with the shortest recovered screen time in our sample:

```{r}
new_screen_time |> arrange(screen_time) |> head(n = 6)
```

Some of the shortest recovered values come from the decimal-hours branch (a bare number like `0.29`, multiplied by 60). Unlike the hours`'`minutes cases, there is no independent check on these -- a genuinely tiny decimal and a mistyped one look identical once parsed as a number. Because we are not completely sure, we will leave them as reported. The object `new_screen_time` contains our final solution for this case study.
...
One of the datasets provided in **behdslabs** shows scientific funding
...
The steps above can actually be skipped because we include this raw data in the **behdslabs** package as well.
...
Suppose we want to show daily screen-time trends by country for the
Caribbean. If we make this plot

```{r caribbean}
global_tech_adoption |>
  filter(region == "Caribbean") |>
  ggplot(aes(year, avg_daily_screen_time_hours, color = country)) +
  geom_line()
```
...
x <- levels(global_tech_adoption$country)
levels(global_tech_adoption$country) <- case_when(
...
#| eval: false
library(forcats)
global_tech_adoption$country <-
  fct_recode(global_tech_adoption$country,
...
2\. In the `extdata` directory of the **behdslabs** package, you will find
