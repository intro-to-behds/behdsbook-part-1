...
library(tidyverse)
library(behdslabs)
product_launch_forecast$startdate |> head()
```

The dates look like strings, but they are not:

```{r}
class(product_launch_forecast$startdate)
```

Look at what happens when we convert them to numbers:

```{r}
as.numeric(product_launch_forecast$startdate) |> head()
```
...
```{r raw-prefs-vs-time}
product_launch_forecast |> filter(panel == "Panel 081" & market == "U.S.") |>
  ggplot(aes(startdate, raw_pref_current)) +
  geom_line()
```
...
```{r}
set.seed(2002)
dates <- sample(product_launch_forecast$startdate, 10) |> sort()
dates
```
...
Another very useful function is `round_date`. It can be used to _round_ dates to nearest year, quarter,  month, week, day, hour, minutes, or seconds. So if we want to group all the preference surveys by week of the year we can do the following:

```{r pref-margin-versus-week}
product_launch_forecast |>
  mutate(week = round_date(startdate, "week")) |>
  group_by(week) |>
  summarize(margin = mean(raw_pref_new - raw_pref_current)) |>
...
```{r}
object.size(product_launch_forecast$startdate)
object.size(as.IDate(product_launch_forecast$startdate))
```
